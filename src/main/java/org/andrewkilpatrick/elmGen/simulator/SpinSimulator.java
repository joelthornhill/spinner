...
public class SpinSimulator extends Thread {
	 ...
	 */
	public SpinSimulator(SpinProgram prog, String inputFilename,
	...
	public void simulate(int tailout, AudioSource input,
		...
		while (runSimulator) {
			...
			if (ret < 1) {
				...
				for (int i = 0; i < ret; i += 2) {
					...
					state.setRegVal(SpinProgram.ADCL, inBuf[i]);
					state.setRegVal(SpinProgram.ADCR, inBuf[i + 1]);
					state.setRegVal(SpinProgram.POT0, pot0);
					state.setRegVal(SpinProgram.POT1, pot1);
					state.setRegVal(SpinProgram.POT2, pot2);
					processSample();
					outBuf[i] = state.getRegVal(SpinProgram.DACL);
					outBuf[i + 1] = state.getRegVal(SpinProgram.DACR);
		...
		while (tailoutSamps < tailout) {
			for (int i = 0; i < outBuf.length; i += 2) {
				state.setRegVal(SpinProgram.ADCL, 0);
				state.setRegVal(SpinProgram.ADCR, 0);
				state.setRegVal(SpinProgram.POT0, pot0);
				state.setRegVal(SpinProgram.POT1, pot1);
				state.setRegVal(SpinProgram.POT2, pot2);
				processSample();
