...
public class LoadSinLFO extends Instruction {
	...
	public void simulate(SimulatorState state) {
		if(lfo == 1) {
			state.setRegVal(SpinProgram.SIN1_RATE, (freq & 0x1ff) << 14);
			state.setRegVal(SpinProgram.SIN1_RANGE, (amp & 0x7fff) << 8);
			state.jamSinLFO(1);			
		}
		else {
			state.setRegVal(SpinProgram.SIN0_RATE, (freq & 0x1ff) << 14);
			state.setRegVal(SpinProgram.SIN0_RANGE, (amp & 0x7fff) << 8);
			state.jamSinLFO(0);	
		}
	}
}
