package example
import example.EquParser.{EquDoubleValue, EquIntegerValue, EquStringValue, EquValue}
...
class Instructions extends ElmProgram("Parser") {
  ...
  def getInt(addr: MapsToInteger, map: Map[String, EquValue]): Int = {
    addr match {
      ...
      case s: StringValue => map.get(s.value.stripPrefix("-")).map {
  ...
  case class Rdax(addr: MapsToInteger, scale: MapsToDouble) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"readRegister(${getInt(addr, map)}, ${getDouble(scale, map)})"
  ...
  case class Rda(memName: String, offset: Double, scale: MapsToDouble) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"readDelay($memName, $offset, ${getDouble(scale, map)})"
  ...
  case class Wrax(addr: MapsToInteger, scale: MapsToDouble) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"writeRegister(${getInt(addr, map)}, ${getDouble(scale, map)})"
...
//    def run(map: Map[String, EquValue]) = writeAllpass(getInt(addr, map), getDouble(scale, map))
    ...
    def runString(map: Map[String, EquValue]) = s"writeAllpass($memName, $offset, ${getDouble(scale, map)})"
...
//    def run(map: Map[String, EquValue]) = writeDelay(getInt(addr, map), getDouble(scale, map))
    ...
    def runString(map: Map[String, EquValue]) = s"writeDelay($memName, $offset, ${getDouble(scale, map)})"
  ...
  case class Sof(scale: MapsToDouble, offset: MapsToDouble) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"scaleOffset(${getDouble(scale, map)}, ${getDouble(offset, map)})"
  ...
  case class Skp(flags: MapsToInteger, nSkip: MapsToInteger) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"skip(${getInt(flags, map)}, ${getInt(nSkip, map)})"
  ...
  case class Exp(scale: MapsToDouble, offset: MapsToDouble) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"exp(${getDouble(scale, map)}, ${getDouble(offset, map)})"
  ...
  case class Wldr(lfo: MapsToInteger, freq: MapsToInteger, amp: MapsToInteger) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"loadRampLFO(${getInt(lfo, map)}, ${getInt(freq, map)}, ${getInt(amp, map)})"
  ...
  case class ChoRda(lfo: MapsToInteger, flags: MapsToInteger, addr: MapsToInteger) extends Instruction {
    ...
    def runString(map: Map[String, EquValue]) = s"chorusReadDelay(${getInt(lfo, map)}, ${getInt(flags, map)})"

    override def toString: String = s"chorusReadDelay($lfo, $flags, $addr)"
  }

  case class ChoSof(lfo: MapsToInteger, flags: MapsToInteger, offset: MapsToDouble) extends Instruction {
    def run(map: Map[String, EquValue]) = chorusScaleOffset(getInt(lfo, map), getInt(flags, map), getDouble(offset, map))
    def runString(map: Map[String, EquValue]) = s"chorusScaleOffset(${getInt(lfo, map)}, ${getInt(flags, map)}, ${getDouble(offset, map)})"
