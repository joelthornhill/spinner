...
public class Skip extends Instruction {
	...
	public void simulate(SimulatorState state) {
		...
		else if((flags & SpinProgram.ZRC) > 0) {
		...
		else if((flags & SpinProgram.ZRO) > 0) {
		...
		else if((flags & SpinProgram.GEZ) > 0) {
			if(state.getACCVal() > 0) {
				skip = true;
			}
			else {
		...
		else if((flags & SpinProgram.NEG) > 0) {
			if(state.getACCVal() < 0) {
				skip = true;
			}
			else {
