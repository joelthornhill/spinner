...
public class LoadRampLFO extends Instruction {
	...
	public void simulate(SimulatorState state) {
		...
		if(lfo == 1) {
			state.setRegVal(SpinProgram.RMP1_RATE, regFreq);
			state.setRegVal(SpinProgram.RMP1_RANGE, amp);
			state.jamRampLFO(1);
		}
		else {
			state.setRegVal(SpinProgram.RMP0_RATE, regFreq);
			state.setRegVal(SpinProgram.RMP0_RANGE, amp);
			state.jamRampLFO(0);
		}
	}
